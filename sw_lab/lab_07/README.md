# CS-181/DA-210 Software Lab 7

The goal of this SW Lab is to learn how to work with JSON-formatted files and JSON procedural programming to create `pandas` `DataFrame`s from JSON-formatted data, as well as how to create nodes and update node attributes in JSON-formatted data.
...
## Step 1: JSON procedural programming

Complete Parts A-C of [json_programming.ipynb](json_programming.ipynb) to get some practice processing JSON-formatted data.
...
- Use the terminal command `git add <filepath>` for each file that you've changed and want to submit.  For this lab and the previous, you'll likely want the following (remember that `$` represents the prompt, so you don't type that part):
    ...
    ```
    $ git add sw_lab/lab_07/json_programming.ipynb
    ```
...
- Use the terminal command `git commit -m "<commit message>"` to create a local commit.  For this lab, you could use:
    ...
    ```
    $ git commit -m "Completed SW Lab 7."
    ```
