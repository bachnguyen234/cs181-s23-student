...
The goal of this SW Lab is to gain more practice with Application Programming Interface (API) providers via a GitHub API case study.
...
Complete Parts A-D of [`apis2.ipynb`](apis2.ipynb).  This notebook contains exercises that walk you through a case study of data acquisition via the GitHub API.
...
- Use the terminal command `git add <filepath>` for each file that you've changed and want to submit.  For this lab and the previous, you'll likely want the following (remember that `$` represents the prompt, so you don't type that part):
    ...
    ```
    $ git add sw_lab/lab_14/apis2.ipynb
    ```
