# CS-181/DA-210 Homework 3.c

The goal of this HW is to help you practice working with XML operations using the `etree` module in Python.
...
Complete [`xml_ops.ipynb`](xml_ops.ipynb).  This notebook contains exercises using the `etree` module to work with XML-encoded text files.
...
- Use the terminal command `git add <filepath>` for each file that you've changed and want to submit.  For this homework, you'll likely want the following (remember that `$` represents the prompt, so you don't type that part):
    ...
    ```
    $ git add hw/HW_3.c/xml_ops.ipynb
    ```
