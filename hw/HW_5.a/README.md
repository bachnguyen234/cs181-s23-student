...
The goal of this HW is to help you practice with HTTP request and response messages.

**Note that you may complete this homework with one partner.  You must type up all of the code yourself, but you can work together and look at each other's code.  You _must_ include the name of your partner, if any, in the cell at the end of http_intro.ipynb.**

---

## Step 1: HTTP

Complete Parts A-E of [`http_intro.ipynb`](http_intro.ipynb).  This notebook contains exercises that help you explore HTTP request and response messages.
...
- Use the terminal command `git add <filepath>` for each file that you've changed and want to submit.  For this homework, you'll likely want the following (remember that `$` represents the prompt, so you don't type that part):
    ...
    ```
    $ git add hw/HW_5.a/http_intro.ipynb
    ```
