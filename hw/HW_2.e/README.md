# CS-181/DA-210 Homework 2.e

The goal of this HW is to help you practice operations to combine tables in `pandas`.

**Note that you may complete this homework with one partner.  You must type up all of the code yourself, but you can work together and look at each other's code.  You _must_ include the name of your partner, if any, in the cell at the end of combine.ipynb.**

---

## Step 1: Combining tables

Complete [`combine.ipynb`](combine.ipynb).  This notebook contains exercises using `concat()`, `join()`, and `merge()` to combine two `pandas DataFrame`s into one.
...
- Use the terminal command `git add <filepath>` for each file that you've changed and want to submit.  For this homework, you'll likely want the following (remember that `$` represents the prompt, so you don't type that part):
    ...
    ```
    $ git add hw/HW_2.e/combine.ipynb
    ```
